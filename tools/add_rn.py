#!/usr/bin/env python3
"""
add_rn.py — herramienta defensiva para crear/obsoletar reglas de negocio (RN-xxx)
en docs/business-rules.md de forma atómica y validada.

Implementa P5 (No se borra historia) y P6 (Referencias estables) de AGENTS.md.
...
Sale con código distinto de cero y no toca el archivo si cualquier validación falla.
"""
...
def _display_path() -> str:
    """Ruta legible de BUSINESS_RULES_PATH para mensajes de stdout."""
    try:
...
def _atomic_write(content: str) -> None:
    """Escribe de forma atómica: archivo temporal + os.replace."""
...
def cmd_deprecate(args: argparse.Namespace) -> int:
    ...
    if not args.reemplazada_por:
        raise ValidationError(
            ...
            "'obsoleta' siempre registra qué RN la reemplaza."
        )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Herramienta defensiva para crear/obsoletar RN en docs/business-rules.md"
    )
